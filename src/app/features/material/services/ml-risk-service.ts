...
export class MLRiskService {
  ...
  private initAutoRefresh(): void {
    this.autoRefresh$.pipe(
      switchMap(() => this.checkMLApiHealth()),
      switchMap(isAvailable => isAvailable ? this.getAllMaterialsWithRisk() : of([]))
    ).subscribe({
      next: (results) => {
        if (results.length) console.log('✅ Risk cache auto-refreshed');
      },
      error: (err) => {
  ...
  checkMLApiHealth(): Observable<boolean> {
    return this.http.get<any>(`${this.springApiUrl}/risk-summary`).pipe(
      map(() => true),
      catchError(() => of(false))
    );
  }
