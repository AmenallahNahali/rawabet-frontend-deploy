...
export class LoyaltyComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.loadDashboard();
    this.connectLoyaltyWS();
  }

  ngOnDestroy(): void {
    this.wsSub?.unsubscribe();
    this.carteService.disconnectLoyalty();
  }

  private connectLoyaltyWS(): void {
    const userId = this.authService.getCurrentUserId();
    const token  = this.authService.getToken();
    if (!userId) return;

    this.carteService.connectLoyalty(userId, token);

    this.wsSub = this.carteService.loyaltyUpdate$.subscribe((updated) => {
      this.carte = updated;

      // Flash animation to catch user attention
      this.pointsFlash = true;
      setTimeout(() => { this.pointsFlash = false; this.cdr.detectChanges(); }, 1500);

      this.isLive = true;

      // Reload history so the new BONUS entry appears
      this.carteService.getHistory()
        .pipe(map((res: any) => Array.isArray(res?.content) ? res.content : (Array.isArray(res) ? res : [])))
        .subscribe({ next: (h) => { this.history = h; this.cdr.detectChanges(); } });
