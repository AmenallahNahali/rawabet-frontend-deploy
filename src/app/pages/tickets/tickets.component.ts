...
export class TicketsComponent implements OnInit {
  ...
  // FIX: start as true so template shows loading immediately
  loading = true;
  seatsLoading = false;
  submitting = false;
  ...
  ngOnInit(): void {
    ...
    // FIX: single entry point, loading already true from declaration
  ...
  private initializeCurrentUser(): void {
    ...
    if (tokenUserId) {
      // User ID already in token — no need to call /me
    ...
    // FIX: only call /me if authenticated, loading stays true while waiting
  ...
  private getReservationErrorMessage(err: any, fallbackMessage: string): string {
    ...
    if (normalizedMessage.includes('seat already reserved')) {
