...
export class AdminMaterielsUnifiedComponent implements OnInit, OnDestroy {
  ...
  get filteredCategories(): CategorieMateriel[] {
    const q = this.categoriesSearchQuery.toLowerCase();
  ...
  loadMaterielsByCategory(categoryId: number): void {
    ...
    this.materielService.getMaterielsByCategorie(categoryId).subscribe({
      next: (materials) => {
        this.materiels = materials;
        this.filterMateriels();
        this.materielsLoading = false;
        this.cdr.detectChanges();

        // Load risk data in background — does not block the table
        this.mlRiskService.getAllMaterialsWithRisk().subscribe({
          next: (risks) => {
  ...
  openEditMaterielModal(materiel: Materiel): void {
    ...
    this.materielForm = {
      ...
      quantiteTotale: materiel.quantiteTotale ?? materiel.quantiteDisponible ?? 0,
