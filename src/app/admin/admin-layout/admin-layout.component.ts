...
export class AdminLayoutComponent implements OnInit, OnDestroy {
  ...
  private notifCountSub?: Subscription;
  ...
  private readonly allMenuItems: AdminMenuItem[] = [
    ...
    { label: 'Event Spaces',  icon: '🏛️', route: '/admin/eventspaces',   roles: ['SUPER_ADMIN', 'ADMIN_EVENT'] },
    { label: 'Films',         icon: '🎬', route: '/admin/films',          roles: ['SUPER_ADMIN', 'ADMIN_CINEMA'] },
    { label: 'Cinemas',       icon: '🎭', route: '/admin/cinemas',        roles: ['SUPER_ADMIN', 'ADMIN_CINEMA'] },
  ...
  ngOnInit(): void {
    // Subscribe to unread count (updated by WS in real-time)
    this.notifCountSub = this.notificationService.unreadCount$.subscribe((count) => {
    ...
    this.userService.getMe().subscribe({
      next: (u: any) => {
        ...
        // Connect WebSocket after we have the adminId
        if (this.adminId) {
    ...
    // Also try to connect immediately with the token (adminId from constructor)
    if (this.adminId) {
      this.connectNotifications();
    }
  }
  ...
  private connectNotifications(): void {
    ...
    // Load current unread count from HTTP first
    this.notificationService.getUnreadCount(this.adminId).subscribe({
      next: (res: any) => {
        this.notificationService.setUnreadCount(res?.count || res?.unreadCount || 0);
        this.cdr.markForCheck();
      }
    });

    // Open real-time WebSocket connection
    this.notificationService.connect(this.adminId, token);
  }
